...
/**
 * errors - errors are printed based on case
...
void errors(int error)
{
	switch (error)
	{
	case 1: /* prints fork error to stderr */
		write(STDERR_FILENO, ERR_FORK, _strlen(ERR_FORK));
		perror("Error");
		break;

	case 2: /* prints execve error to stderr */
		perror("Error");
		break;

	case 3: /* prints malloc error to stderr */
		write(STDERR_FILENO, ERR_MALLOC, _strlen(ERR_MALLOC));
		break;
	case 4: /* prints empty path error to stderr */
